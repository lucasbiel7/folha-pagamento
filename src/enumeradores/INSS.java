...
public enum INSS {
    ...
    public static INSS categoria(double salario) {
        return Arrays.asList(values()).stream().filter(t -> t.getLimite().getMinimo() <= salario && t.getLimite().getMaximo() >= salario).findFirst().orElse(CATEGORIA3);
    }
